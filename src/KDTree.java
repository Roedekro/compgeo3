...
public class KDTree {
    ...
    public static KDTreeNode recurBuildKDTree(Range parentRange, PointCollection points, int depth) {
        ...
        if(pair.right.pointsX.size() > 0) {
        	vRight = recurBuildKDTree(rightRange, pair.right, depth+1);
        }
    ...
    public static List<Point> searchKDTree(KDTreeNode root, Range range) {
        ...
        if (root.leftChild != null && range.contains(root.leftChild.region)) {
            leftPoints = getLeafNodes(root.leftChild);
        } else if (root.leftChild != null && root.leftChild.region.intersects(range)) {
        ...
        if (root.rightChild != null && range.contains(root.rightChild.region)) {
            rightPoints = getLeafNodes(root.rightChild);
        } else if (root.rightChild != null && root.rightChild.region.intersects(range)) {
